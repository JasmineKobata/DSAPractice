class Solution {
    public int nearestExit(char[][] maze, int[] entrance) {
        ...
        List<Integer> list = Arrays.stream(entrance).boxed().toList();
        visited.add(list);
    ...
    public int bfs(char[][] maze, Set<List<Integer>> visited, int[] currentSqr) {
        ...
        if (x-1 >= 0 && !visited.contains(Arrays.asList(left))) {
            return 1 + bfs(maze, visited, left);
        }
        if (y-1 >= 0 && !visited.contains(Arrays.asList(up))) {
            return 1 + bfs(maze, visited, up);
        }
        if (x+1 >= 0 && !visited.contains(Arrays.asList(right))) {
            return 1 + bfs(maze, visited, right);
        }
        if (y+1 >= 0 && !visited.contains(Arrays.asList(down))) {
